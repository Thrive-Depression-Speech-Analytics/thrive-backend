...
const authService = {
	...
	analyzeHandler: async (request, h) => {
		try {
			...
			const depressionLevel = analysisResult.depressionLevel;

			const suggestions = generateSuggestions(depressionLevel);
...
async function analyzeAudio(audioFile) {
    try {
        ...
        const bucketName = process.env.GCS_BUCKET_NAME; // Nama bucket GCS 

        // Unggah audio ke Google Cloud Storage
        const bucket = storage.bucket(bucketName);
        const fileUpload = bucket.file(audioFile.originalFilename);
        await fileUpload.save(audioFile.filepath);

        // Dapatkan URL publik dari audio yang diupload
        const audioUrl = await fileUpload.getSignedUrl({
            action: "read",
            expires: "03-09-2491", // Atur tanggal kedaluwarsa 
        });

        // Kirim URL audio ke model ML (REST API)
        const response = await fetch("https://your-ml-model-api-endpoint", {
            method: "POST",
            body: JSON.stringify({ audioUrl: audioUrl[0] }), // Kirim URL audio
        ...
        fs.unlinkSync(audioFile.filepath); // Hapus file sementara (jika belum dihapus)
    }
}
